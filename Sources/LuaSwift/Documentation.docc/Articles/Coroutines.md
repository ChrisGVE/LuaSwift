...
Lua coroutines provide cooperative multitasking within a single Lua state. ``LuaEngine`` provides methods to create, resume, and manage coroutines through the ``CoroutineHandle`` type.
...
``CoroutineHandle`` instances belong to their parent engine:
