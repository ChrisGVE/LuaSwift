...
Safely use LuaSwift across threads with comprehensive thread safety guarantees.

## Overview

``LuaEngine`` is designed for safe concurrent access. Every public method acquires an internal lock before accessing the Lua state, ensuring thread-safe operation.
...
- No data races on the Lua state
- Safe registration of functions and value servers
