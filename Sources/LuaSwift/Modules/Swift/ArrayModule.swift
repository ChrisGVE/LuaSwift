...
//
  ...
  public struct ArrayModule {
    ...
    // ArrayDType and NDArray (ArrayData) are now in the ArraySwift package
    ...
    private static func extractArrayData(_ value: LuaValue) throws -> ArrayData {
      ...
      guard data.count == expectedSize else {
        throw LuaError.callbackError(
          "array: data size \(data.count) doesn't match shape \(shape) (expected \(expectedSize))")
      }
      ...
      if dtype.isComplex {
        ...
        guard imag.count == expectedSize else {
          throw LuaError.callbackError(
            "array: imag size \(imag.count) doesn't match shape \(shape) (expected \(expectedSize))"
          )
        }
    ...
    private static func complexArrayCallback(_ args: [LuaValue]) throws -> LuaValue {
      ...
      guard realArray.shape == imagArray.shape else {
        throw LuaError.callbackError(
          "array.complex_array: real and imag arrays must have same shape")
      }

      try trackArrayAllocation(count: realArray.size * 2)
      return createArrayTable(
        ArrayData.complexArray(
    ...
    private static func fromPolarCallback(_ args: [LuaValue]) throws -> LuaValue {
      ...
      guard magArray.shape == phaseArray.shape else {
        throw LuaError.callbackError(
          "array.from_polar: magnitude and phase arrays must have same shape")
      }
    ...
    private static func getCallback(_ args: [LuaValue]) throws -> LuaValue {
      ...
      guard indices.count == arrayData.ndim else {
        throw LuaError.callbackError(
          "array.get: expected \(arrayData.ndim) indices, got \(indices.count)")
      }
    ...
    private static func setCallback(_ args: [LuaValue]) throws -> LuaValue {
      ...
      guard indices.count == arrayData.ndim else {
        throw LuaError.callbackError(
          "array.set: expected \(arrayData.ndim) indices, got \(indices.count)")
      }
    ...
    private static func reshapeCallback(_ args: [LuaValue]) throws -> LuaValue {
      ...
      guard newSize == arrayData.size else {
        throw LuaError.callbackError(
          "array.reshape: cannot reshape array of size \(arrayData.size) to shape \(newShape)")
      }
    ...
    private static func broadcastToComplex(_ array: ArrayData, shape targetShape: [Int])
      -> ArrayData
    {
      ...
      let realBroadcast = broadcastTo(
        ArrayData(shape: array.shape, data: array.real), shape: targetShape)
      let imagData = array.imag ?? [Double](repeating: 0, count: array.size)
      let imagBroadcast = broadcastTo(
        ArrayData(shape: array.shape, data: imagData), shape: targetShape)

      return ArrayData.complexArray(
        shape: targetShape, real: realBroadcast.real, imag: imagBroadcast.real)
    }
    ...
    private static func binaryOp(_ args: [LuaValue], op: (Double, Double) -> Double, name: String)
      throws -> LuaValue
    {
      guard args.count >= 2 else {
    ...
    private static func sqrtCallback(_ args: [LuaValue]) throws -> LuaValue {
      ...
      if arrayData.isComplex {
        ...
        return createArrayTable(
          ArrayData.complexArray(shape: arrayData.shape, real: realResult, imag: imagResult))
      } else {
    ...
    private static func reduceAlongAxis(
      _ arrayData: ArrayData, axis: Int, reduction: ([Double]) -> Double
    ) throws -> LuaValue {
    ...
    private static func comparisonOp(
      _ args: [LuaValue], op: (Double, Double) -> Double, name: String
    ) throws -> LuaValue {
      guard args.count >= 2 else {
        throw LuaError.callbackError("array.\(name): requires two arguments")
      }
    ...
    private static func whereCallback(_ args: [LuaValue]) throws -> LuaValue {
      ...
      if isScalarX {
        let scalar = args[1].numberValue!
        xData = ArrayData(
          shape: condData.shape, data: [Double](repeating: scalar, count: condData.size))
      } else {
    ...
    private static func cumulativeAlongAxis(
      _ arrayData: ArrayData, axis: Int, initialValue: Double, op: (Double, Double) -> Double
    ) throws -> LuaValue {
    ...
    private static func nonzeroCallback(_ args: [LuaValue]) throws -> LuaValue {
      ...
      var result: [LuaValue] = []
      for d in 0..<arrayData.ndim {
        let dimArray = createArrayTable(
          ArrayData(shape: [dimIndices[d].count], data: dimIndices[d]))
      ...
      for arr in arrays.dropFirst() {
        ...
        for d in 0..<ndim where d != axis0 {
          guard arr.shape[d] == baseShape[d] else {
            throw LuaError.callbackError(
              "array.concatenate: arrays must match on all axes except concatenation axis")
          }
        }
      }
      ...
      if mode == "constant" {
        ...
        for dstIdx in 0..<resultSize {
          ...
          for d in 0..<arrayData.ndim {
            ...
            let srcCoord =
              ((shifted % arrayData.shape[d]) + arrayData.shape[d]) % arrayData.shape[d]
            srcIdx += srcCoord * srcStrides[d]
          }
    ...
    private static func insertCallback(_ args: [LuaValue]) throws -> LuaValue {
      ...
      if let singleVal = args[2].numberValue {
        ...
        // Could be array data
        if valTable["shape"] != nil, valTable["data"] != nil {
      ...
      func computeGradientAlongAxis(_ axis0: Int) -> ArrayData {
        ...
        for flatIdx in 0..<arrayData.size {
          ...
          if axisSize == 1 {
            ...
            let nextIdx = flatIdx + axisStride
            resultData[flatIdx] =
              (arrayData.data[nextIdx] - arrayData.data[prevIdx]) / (2 * spacing)
          }
        }
