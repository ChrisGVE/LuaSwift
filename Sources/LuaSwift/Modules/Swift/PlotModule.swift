...
//
  ...
  public struct PlotModule: LuaSwiftModule {
    ...
    public static func install(in engine: LuaEngine) throws {
      ...
      engine.registerFunction(
        name: "_luaswift_plot_destroy_context", callback: destroyContextCallback)
    ...
    private static let toSVGCallback: ([LuaValue]) throws -> LuaValue = { args in
      ...
      private static let toPNGCallback: ([LuaValue]) throws -> LuaValue = { args in
        ...
        guard
          let data = context.renderToPNG(size: CGSize(width: width, height: height), scale: scale)
        else {
          return .nil
        }
      ...
      private static let saveFigCallback: ([LuaValue]) throws -> LuaValue = { args in
        ...
        default:  // png
          let dpi =
            args.count > 4 && args[4].tableValue?["dpi"]?.numberValue != nil
    ...
    private static let plotLuaWrapper = """
      ...
      function Axes:plot(x, y, fmt_or_opts, opts)
          ...
          if #x > 0 then
              ...
              -- Scale-aware transform functions
              local tx, ty = self:_make_transforms(xmin, xmax, ymin, ymax)
      ...
      function Axes:bar(x, height, opts)
          ...
          -- Scale-aware y-transform
          local _, ty_bar = self:_make_transforms(1, n, 0, hmax)
          ...
          for i = 1, n do
              ...
              local bot = bottom_is_array and bottom[i] or bottom
              local by = ty_bar(bot) or (margin + (bot / hmax) * plot_height)
              local by_top = ty_bar(height[i] + bot) or (margin + ((height[i] + bot) / hmax) * plot_height)
              local bh = by_top - by
      ...
      function Axes:hist(data, opts)
          ...
          local tx_hist, ty_hist = self:_make_transforms(dmin, dmax, 0, cmax)
          ...
          for i = 1, bins do
              local bx = tx_hist(bin_edges[i]) or (margin + (i - 1) * bar_width)
              local bx_next = tx_hist(bin_edges[i + 1]) or (margin + i * bar_width)
              bar_width = bx_next - bx
              local by = margin
              local bh = (ty_hist(counts[i]) or (margin + counts[i] / cmax * plot_height)) - margin
      ...
      function Axes:_make_transforms(xmin, xmax, ymin, ymax)
          ...
          local xscale = self:get_xscale()
          local yscale = self:get_yscale()
          local xlt = self._xscale_linthresh
          local ylt = self._yscale_linthresh

          local sxmin = apply_scale(xmin, xscale, xlt) or 0
          local sxmax = apply_scale(xmax, xscale, xlt) or 1
          local symin = apply_scale(ymin, yscale, ylt) or 0
          local symax = apply_scale(ymax, yscale, ylt) or 1

          if sxmax == sxmin then sxmax = sxmin + 1 end
          if symax == symin then symax = symin + 1 end

          local function tx(v)
              local sv = apply_scale(v, xscale, xlt)
              if sv == nil then return nil end
              return margin + (sv - sxmin) / (sxmax - sxmin) * plot_width
          end

          local function ty(v)
              local sv = apply_scale(v, yscale, ylt)
              if sv == nil then return nil end
              return margin + (sv - symin) / (symax - symin) * plot_height
          end

          return tx, ty, sxmin, sxmax, symin, symax
      end

      -- Generate tick positions for a given scale.
      local function generate_ticks(vmin, vmax, scale, linthresh, n_ticks)
          n_ticks = n_ticks or 8
          local ticks = {}
          if scale == "log" then
              if vmin <= 0 then vmin = 1e-10 end
              local lo = math.floor(log10(vmin))
              local hi = math.ceil(log10(vmax))
              for e = lo, hi do
                  local v = pow10(e)
                  if v >= vmin and v <= vmax then
                      table.insert(ticks, v)
                  end
              end
              if #ticks < 2 then
                  ticks = {}
                  local step = (vmax - vmin) / n_ticks
                  for i = 0, n_ticks do
                      table.insert(ticks, vmin + i * step)
                  end
              end
          elseif scale == "symlog" then
              linthresh = linthresh or 1
              -- Linear ticks near zero
              if vmin < linthresh and vmax > -linthresh then
                  local lin_lo = math.max(vmin, -linthresh)
                  local lin_hi = math.min(vmax, linthresh)
                  local nlin = 3
                  for i = 0, nlin do
                      table.insert(ticks, lin_lo + i / nlin * (lin_hi - lin_lo))
                  end
              end
              -- Log ticks for positive region
              if vmax > linthresh then
                  local lo = math.floor(log10(linthresh))
                  local hi = math.ceil(log10(vmax))
                  for e = lo, hi do
                      local v = pow10(e)
                      if v > linthresh and v <= vmax then
                          table.insert(ticks, v)
                      end
                  end
              end
              -- Log ticks for negative region
              if vmin < -linthresh then
                  local lo = math.floor(log10(linthresh))
                  local hi = math.ceil(log10(-vmin))
                  for e = lo, hi do
                      local v = -pow10(e)
                      if v < -linthresh and v >= vmin then
                          table.insert(ticks, v)
                      end
                  end
              end
      ...
      -- Format tick label for a given scale.
      local function format_tick(v, scale)
          if scale == "log" then
              local e = log10(v)
              if e == math.floor(e) then
                  if e == 0 then return "1" end
                  return "10^" .. tostring(math.floor(e))
              end
          end
          return string.format("%.4g", v)
      end

      -- Draw axis ticks and labels. Called from render or plot functions.
      function Axes:_draw_ticks(ctx, xmin, xmax, ymin, ymax)
          local margin = 50
          local plot_width = self._figure._width - 2 * margin
          local plot_height = self._figure._height - 2 * margin
          local xscale = self:get_xscale()
          ...
          ctx:set_stroke("#333333", 1)

          -- X-axis ticks
          local xticks = generate_ticks(xmin, xmax, xscale, xlt)
          for _, v in ipairs(xticks) do
              local px = tx(v)
              if px then
                  ctx:move_to(px, margin)
                  ctx:line_to(px, margin - 5)
                  ctx:stroke()
                  ctx:text(format_tick(v, xscale), px - 10, margin - 8, {fontSize = 9})
              end
          end

          -- Y-axis ticks
          local yticks = generate_ticks(ymin, ymax, yscale, ylt)
          for _, v in ipairs(yticks) do
              local py = ty(v)
              if py then
                  ctx:move_to(margin, py)
                  ctx:line_to(margin - 5, py)
                  ctx:stroke()
                  local label = format_tick(v, yscale)
                  ctx:text(label, margin - 8 - #label * 5, py - 4, {fontSize = 9})
              end
          end
      end
      ...
      function Axes:boxplot(data, opts)
          ...
          for idx, d in ipairs(datasets) do
              ...
              -- Scale-aware y-transform
              local _, ty = self:_make_transforms(1, n + 1, gmin, gmax)
