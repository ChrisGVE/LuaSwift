  ...
  final class ArrayModuleTests: XCTestCase {
    ...
    func testBroadcastIncompatibleShapesError() throws {
      ...
      do {
        ...
        XCTAssertTrue(
          errorMessage.contains("broadcast"), "Error should mention broadcasting: \(errorMessage)")
      }
    }
    ...
    func testArraySliceBasic() throws {
      let result = try engine.evaluate(
        """
        local a = luaswift.array.array({10, 20, 30, 40, 50})
        local s = a:slice(2, 4)
        return s:tolist()
        """)
      let list = result.arrayValue!.compactMap { $0.numberValue }
      XCTAssertEqual(list, [20, 30, 40])
    }

    func testArraySliceToEnd() throws {
      let result = try engine.evaluate(
        """
        local a = luaswift.array.array({10, 20, 30, 40, 50})
        local s = a:slice(3)
        ...
        """)
      let list = result.arrayValue!.compactMap { $0.numberValue }
      XCTAssertEqual(list, [30, 40, 50])
    }

    func testArraySliceNegativeStart() throws {
      let result = try engine.evaluate(
        """
        local a = luaswift.array.array({10, 20, 30, 40, 50})
        local s = a:slice(-3)
    ...
    func testArraySliceNegativeStop() throws {
      let result = try engine.evaluate(
        """
        local a = luaswift.array.array({10, 20, 30, 40, 50})
        local s = a:slice(1, -2)
        ...
        """)
      let list = result.arrayValue!.compactMap { $0.numberValue }
      XCTAssertEqual(list, [10, 20, 30, 40])
    }

    func testArraySliceWithStep() throws {
      let result = try engine.evaluate(
        """
        local a = luaswift.array.array({10, 20, 30, 40, 50, 60})
        local s = a:slice(1, 6, 2)
        return s:tolist()
        """)
      let list = result.arrayValue!.compactMap { $0.numberValue }
      XCTAssertEqual(list, [10, 30, 50])
    }

    func testArraySliceReverseStep() throws {
      let result = try engine.evaluate(
        """
        local a = luaswift.array.array({10, 20, 30, 40, 50})
        local s = a:slice(5, 1, -1)
        return s:tolist()
        """)
      let list = result.arrayValue!.compactMap { $0.numberValue }
      XCTAssertEqual(list, [50, 40, 30, 20, 10])
    }

    func testArraySliceStepZeroError() throws {
      XCTAssertThrowsError(
        try engine.evaluate(
          """
          local a = luaswift.array.array({1, 2, 3})
          return a:slice(1, 3, 0)
          """))
    }

    func testArraySliceOn2DError() throws {
      XCTAssertThrowsError(
        try engine.evaluate(
          """
          local a = luaswift.array.array({{1,2},{3,4}})
