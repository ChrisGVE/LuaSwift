...
//
  ...
  final class MathExprModuleTests: XCTestCase {
    ...
    func testSimplifyConstantFolding() throws {
      let result = try engine.evaluate(
        """
            local eval = luaswift.eval
            local ast = eval.simplify("2 + 3")
            return eval.eval_ast(ast, {})
        """)
      XCTAssertEqual(result.numberValue ?? 0, 5.0, accuracy: 1e-10)
    }

    func testSimplifyConstantFoldingDivision() throws {
      let result = try engine.evaluate(
        """
            local eval = luaswift.eval
            local ast = eval.simplify("6 / 2")
            return eval.eval_ast(ast, {})
        """)
      XCTAssertEqual(result.numberValue ?? 0, 3.0, accuracy: 1e-10)
    }

    func testSimplifyAddZeroRight() throws {
      let result = try engine.evaluate(
        """
            local eval = luaswift.eval
            local ast = eval.simplify("x + 0")
            return eval.eval_ast(ast, {x = 7})
        """)
      XCTAssertEqual(result.numberValue ?? 0, 7.0, accuracy: 1e-10)
    }

    func testSimplifyAddZeroLeft() throws {
      let result = try engine.evaluate(
        """
            local eval = luaswift.eval
            local ast = eval.simplify("0 + x")
            return eval.eval_ast(ast, {x = 5})
        """)
      XCTAssertEqual(result.numberValue ?? 0, 5.0, accuracy: 1e-10)
    }

    func testSimplifyMultiplyByOneRight() throws {
      let result = try engine.evaluate(
        """
            local eval = luaswift.eval
            local ast = eval.simplify("x * 1")
            return eval.eval_ast(ast, {x = 9})
        """)
      XCTAssertEqual(result.numberValue ?? 0, 9.0, accuracy: 1e-10)
    }

    func testSimplifyMultiplyByOneLeft() throws {
      let result = try engine.evaluate(
        """
            local eval = luaswift.eval
            local ast = eval.simplify("1 * x")
            return eval.eval_ast(ast, {x = 4})
        """)
      XCTAssertEqual(result.numberValue ?? 0, 4.0, accuracy: 1e-10)
    }

    func testSimplifyMultiplyByZero() throws {
      let result = try engine.evaluate(
        """
            local eval = luaswift.eval
            local ast = eval.simplify("x * 0")
            return eval.eval_ast(ast, {x = 999})
        """)
      XCTAssertEqual(result.numberValue ?? -1, 0.0, accuracy: 1e-10)
    }

    func testSimplifyDivideByOne() throws {
      let result = try engine.evaluate(
        """
            local eval = luaswift.eval
            local ast = eval.simplify("x / 1")
            return eval.eval_ast(ast, {x = 8})
        """)
      XCTAssertEqual(result.numberValue ?? 0, 8.0, accuracy: 1e-10)
    }

    func testSimplifyPowerOfZero() throws {
      let result = try engine.evaluate(
        """
            local eval = luaswift.eval
            local ast = eval.simplify("x ^ 0")
            return eval.eval_ast(ast, {x = 42})
        """)
      XCTAssertEqual(result.numberValue ?? 0, 1.0, accuracy: 1e-10)
    }

    func testSimplifyPowerOfOne() throws {
      let result = try engine.evaluate(
        """
            local eval = luaswift.eval
            local ast = eval.simplify("x ^ 1")
            return eval.eval_ast(ast, {x = 6})
        """)
      XCTAssertEqual(result.numberValue ?? 0, 6.0, accuracy: 1e-10)
    }

    func testSimplifyDoubleNegation() throws {
      let result = try engine.evaluate(
        """
            local eval = luaswift.eval
            local ast = eval.simplify("-(-x)")
            return eval.eval_ast(ast, {x = 5})
        """)
      XCTAssertEqual(result.numberValue ?? 0, 5.0, accuracy: 1e-10)
    }

    func testSimplifyLikeTerms() throws {
      let result = try engine.evaluate(
        """
            local eval = luaswift.eval
            local ast = eval.simplify("x + x")
            return eval.eval_ast(ast, {x = 3})
        """)
      XCTAssertEqual(result.numberValue ?? 0, 6.0, accuracy: 1e-10)
    }

    func testSimplifyNumericalEquivalenceComplex() throws {
      // Verify that simplified and unsimplified produce same result
      let result = try engine.evaluate(
        """
            local eval = luaswift.eval
            local expr = "x^2 + 0 + 1*x - 0"
            local original = eval._evaluate(expr, {x = 4})
            local ast = eval.simplify(expr)
            local simplified = eval.eval_ast(ast, {x = 4})
            return math.abs(original - simplified) < 1e-10 and 1 or 0
        """)
      XCTAssertEqual(result.numberValue ?? 0, 1.0)
    }
  }
