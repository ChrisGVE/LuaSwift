...
final class StringXModuleTests: XCTestCase {
  ...
  func testAllFunctionsAvailable() throws {
    let functions = [
      ...
      "is_alpha", "is_digit", "is_alnum", "is_space", "is_upper", "is_lower", "is_empty",
      "is_blank",
  ...
  func testSliceBasic() throws {
    let result = try engine.evaluate(
      """
      return stringx.slice("hello", 2, 4)
      """)
    XCTAssertEqual(result.stringValue, "ell")
  }

  func testSliceFromStart() throws {
    let result = try engine.evaluate(
      """
      return stringx.slice("hello", 1, 3)
      """)
  ...
  func testSliceNegativeStop() throws {
    let result = try engine.evaluate(
      """
      return stringx.slice("hello", 1, -2)
      """)
  ...
  func testSliceWithStep() throws {
    let result = try engine.evaluate(
      """
      return stringx.slice("abcdef", 1, 6, 2)
      """)
  ...
  func testSliceReverseStep() throws {
    let result = try engine.evaluate(
      """
      return stringx.slice("hello", 5, 1, -1)
      """)
  ...
  func testSliceEmptyResult() throws {
    ...
    let result = try engine.evaluate(
      """
      return stringx.slice("hello", 4, 2)
      """)
    XCTAssertEqual(result.stringValue, "")
  }

  func testSliceOutOfBounds() throws {
    // Indices beyond length are clamped
    let result = try engine.evaluate(
      """
      return stringx.slice("hello", 1, 99)
      """)
    XCTAssertEqual(result.stringValue, "hello")
  }
  ...
  func testSliceStepZeroError() throws {
    XCTAssertThrowsError(
      try engine.evaluate(
        """
        return stringx.slice("hello", 1, 5, 0)
        """))
  }

  func testSliceMethodSyntax() throws {
    // Verify s:slice() works via metatable after import()
    try engine.run("stringx.import()")
    let result = try engine.evaluate(
      """
      return ("hello"):slice(2, 4)
      """)
    XCTAssertEqual(result.stringValue, "ell")
  }
