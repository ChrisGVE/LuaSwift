...
final class TableXModuleTests: XCTestCase {
  ...
  // MARK: - slice() step support tests

  func testSliceWithPositiveStep() throws {
    let r = try engine.evaluate(
      """
      local t = {10, 20, 30, 40, 50, 60}
      return luaswift.tablex.slice(t, 1, 6, 2)
      """)
    guard let array = r.arrayValue else {
      XCTFail("Expected array result")
      return
    }
    XCTAssertEqual(array.count, 3)
    XCTAssertEqual(array[0].numberValue, 10)
    XCTAssertEqual(array[1].numberValue, 30)
    XCTAssertEqual(array[2].numberValue, 50)
  }

  func testSliceWithNegativeStep() throws {
    let r = try engine.evaluate(
      """
      local t = {10, 20, 30, 40, 50}
      return luaswift.tablex.slice(t, 5, 1, -1)
      """)
    guard let array = r.arrayValue else {
      XCTFail("Expected array result")
      return
    ...
    XCTAssertEqual(array.count, 5)
    XCTAssertEqual(array[0].numberValue, 50)
    XCTAssertEqual(array[4].numberValue, 10)
  }

  func testSliceStepZeroRaisesError() throws {
    XCTAssertThrowsError(
      try engine.evaluate(
        """
        return luaswift.tablex.slice({1, 2, 3}, 1, 3, 0)
        """))
  }

  func testSliceDefaultsForNegativeStep() throws {
    let r = try engine.evaluate(
      """
      local t = {10, 20, 30}
      return luaswift.tablex.slice(t, nil, nil, -1)
      """)
    guard let array = r.arrayValue else {
      XCTFail("Expected array result")
      return
