...
#
# Two dependency groups are modelled in test-matrix.json:
#   - optional_dependencies: sibling-clone ChrisGVE packages (need a git
#     checkout next to LuaSwift). These drive the combinatorial 2^n matrix.
#   - toggle_dependencies: pure SwiftPM deps (Yams, TOMLKit) fetched from a
#     registry — no clone. Each is exercised once at its non-default value
#     (Yams default-ON -> build OFF; TOMLKit default-OFF -> build ON).
#
# When adding a new sibling-clone optional dependency:
#   1. Add it to .optional_dependencies in scripts/test-matrix.json
#   2. Update Package.swift with the same env var pattern
#   3. CI workflow will automatically pick up the new dependency
#
# When adding a new pure-SwiftPM toggle dependency:
#   1. Add it to .toggle_dependencies in scripts/test-matrix.json
#      (with default + test_value, needs_clone:false, no repo)
#   2. Update Package.swift with the same env var pattern
#   3. CI's test-toggles job picks it up automatically
...
# Load toggle dependencies — pure SwiftPM deps (no clone) exercised at their
# non-default value. Each entry is "name:env_suffix:default:test_value", e.g.
# "Yams:YAMS:1:0" (default ON, tested OFF) or "TOMLKit:TOMLKIT:0:1".
TOGGLE_DEPS=()
while IFS= read -r line; do
	TOGGLE_DEPS+=("$line")
done < <(jq -r '.toggle_dependencies[] | "\(.name):\(.env_var | sub("LUASWIFT_INCLUDE_"; "")):\(.default):\(.test_value)"' "$CONFIG_FILE")
...
# Get the number of toggle dependencies (pure SwiftPM, no clone)
get_toggle_count() {
	echo "${#TOGGLE_DEPS[@]}"
}

# Parse toggle entry "Yams:YAMS:1:0" -> a requested field.
# Fields: full (name), env (env suffix), default (default value),
# test (non-default value to exercise).
parse_toggle() {
	local entry="$1"
	local field="$2"
	IFS=':' read -r full env def test <<<"$entry"
	case "$field" in
	full) echo "$full" ;;
	env) echo "$env" ;;
	default) echo "$def" ;;
...
# Run build+test once per toggle dependency at its non-default value, mirroring
# CI's test-toggles job (e.g. LUASWIFT_INCLUDE_YAMS=0, LUASWIFT_INCLUDE_TOMLKIT=1).
# No clone is needed — these are registry-fetched SwiftPM deps.
run_toggle_tests() {
	for entry in "${TOGGLE_DEPS[@]}"; do
		local env_suffix
		local test_value
		local full
		env_suffix=$(parse_toggle "$entry" "env")
		test_value=$(parse_toggle "$entry" "test")
		full=$(parse_toggle "$entry" "full")
		local var_name="LUASWIFT_INCLUDE_$env_suffix"
		echo "Toggle: $var_name=$test_value ($full)"
		env "$var_name=$test_value" swift build
		env "$var_name=$test_value" swift test --skip Benchmark
	done
}
